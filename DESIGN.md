...
One page: primary navigation, cinematic introduction, era navigation, filter toolbar, timeline/chronological-list switch, overview navigator, a chronological full-body character journey gallery with a shared lifespan strip, curated entry points, footer. Memory details use an accessible modal drawer that leaves the selected timeline point exposed. An About dialog explains sources and the scale.
...
## Components
Shared buttons, segmented controls, filter popovers with native checkbox/select affordances, event markers, lane labels, memory drawer, chronology cards, era buttons, and overview brush. Media-backed markers use consistent 256px transparent headshots on matching ink-green circular backplates. Shared points show up to two distinct portraits; count badges continue to mean memories. Dense adjacent portraits may stagger vertically while retaining their exact time coordinate. Unmatched or failed portraits retain the original symbols. A horizontal character gallery includes every supplied identity and uses optimized transparent full-body art. Cards show sourced birth/death labels and sort by birth or earliest dated presence. A restrained four-row (two-row on mobile) lifespan strip follows the cards in view: solid physical-life segments, dashed incomplete/continued presence, open uncertain endpoints, and a shared year axis. Hover/focus highlights overlapping records; selecting a row opens its dates and sources. Compression is limited to empty gaps between physical spans, so Kassandra’s long physical life remains proportional. Cards with records open that person’s complete journey; cards without records show “No memories yet” and open an artwork preview without fabricated dates or changes to the current timeline. CSS custom properties own tokens. Data metadata generates filter options and counts.
...
Dependency-free browser ES modules, HTML, CSS, and SVG; Node built-ins for local development/build/testing. This matches the existing static repository and permits static hosting without a chart license or CDN runtime. Individual character filters include both solo and shared memories, with explicit name aliases and unknown participants preserved. Event CSV values remain authoritative for event points; biography metadata is separately sourced and does not silently reuse speculative deaths or overwrite event dates. Each biography has source URLs, explicit unknowns, and notes for calendar conflicts, long life, suspension, and stored consciousness; a shared dataset path keeps runtime, downloads, build, and tests aligned. Points use Year/Era; a missing Era may be recovered from a matching signed Real Year with visible disclosure. Location and recorded Start/End fields remain available in details and search without changing the point-based chart. Test CSV parsing, date conversion, filtering, scale invertibility, zoom bounds, and real-data loading. Verify browser interactions at desktop/mobile sizes, keyboard access, and console errors.
