...
A single-page Assassin’s Creed timeline built around `data/Assassin's Creed Timeline - Data.csv`. A cinematic introduction leads into an interactive historical atlas, with memory details and a chronological archive.
...
- Combine game, category, and character filters. Search includes titles, descriptions, characters, games, and sources; **/** focuses search.
- Select a memory for its description, date, character, game, and source. Numbered groups open all their memories and offer a closer view.
...
Append rows to the original CSV, retaining its header:

```csv
Year,Approx,Era,Real Year,Category,Character,Game,Source,Title,Image,Description
```

`Year` is a positive integer and `Era` is `BCE` or `CE`. `Approx` accepts `TRUE` or `FALSE`. The plotting date comes from these fields; `Real Year` is retained in the source file but is not needed for calculation. Quote CSV fields containing commas, quotes, or line breaks; double any quote within a quoted field.
...
The initial data contains **48 records**, **13 named titles** (including a Watch Dogs entry), and one unassigned game. The Daniel Cross record at 2000 CE has no title and is shown as “Untitled memory.” The timeline preserves the supplied chronology rather than claiming complete coverage or verifying the underlying lore.
