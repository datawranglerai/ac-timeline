import { loadEvents, filterEvents, formatYear, shortGame } from './data.js';
...
function selectEra(id) {
  ...
  if (id === 'all') state.viewport = [0, 1];
    ...
    if (id === 'isu' && memories.length) focusYears(Math.min(...memories.map((event) => event.year)), Math.max(...memories.map((event) => event.year)), 0.16);
...
async function load() {
  ...
  try {
    const response = await fetch(new URL('../data/Assassin\'s Creed Timeline - Data.csv', import.meta.url));
